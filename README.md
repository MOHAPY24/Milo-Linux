
# MiloLinux core repository

#### This repository contains the core addons and tools for the MiloLinux operating system.

## Contents

- mfget: a simple fetch tool.
- situation - a simple init system (SEE CREDITS!!).
- argon - a Git based package manager (SEE CREDITS!!).
- milo-nuke - emergency system wipe tool.
- musl-libc - rebuilt musl libc for milo.

## Contributing

Feel free to fork and make pull requests.
Make sure to follow the coding style of the existing code.

## License

All tooling is licensed under the BSD-3 Clause, the distro itself is enforced by the Linux kernel's GPLv2 license.
See individual files for license information
